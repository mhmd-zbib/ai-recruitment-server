...
public class ApplicationService {
  ...
  public ApplicationResponse createApplication(
      final ApplicationRequest request, final UUID userId, final UUID jobId) {
    User user =
        userRepository
            .findById(userId)
            .orElseThrow(
    ...
    Application application =
        Application.builder().user(user).job(job).status(ApplicationStatus.SUBMITTED).build();
  ...
  public Page<JobApplicationListResponse> getJobApplications(
      final UUID jobId, final JobApplicationFilter filter, final Pageable pageable) {
    return applicationRepository
        .findAll(JobApplicationSpecification.buildSpecification(jobId, filter), pageable)
  ...
  public Page<ApplicationListResponse> getApplications(
      final UUID userId, final ApplicationFilter filter, final Pageable pageable) {
    return applicationRepository
        .findAll(ApplicationSpecification.buildSpecification(userId, filter), pageable)
