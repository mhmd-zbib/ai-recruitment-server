        ...
        return (root, query, criteriaBuilder) -> {
            ...
            if (filter.getQuery() != null && !filter.getQuery().isEmpty()) {
                ...
                predicates.add(criteriaBuilder.or(
                        SpecificationUtils.createLikePredicate(criteriaBuilder, root.get("firstName"), searchPattern),
                        SpecificationUtils.createLikePredicate(criteriaBuilder, root.get("lastName"), searchPattern),
                        SpecificationUtils.createLikePredicate(criteriaBuilder, root.get("email"), searchPattern),
                        SpecificationUtils.createLikePredicate(criteriaBuilder, jobJoin.get("title"), searchPattern),
                        SpecificationUtils.createLikePredicate(criteriaBuilder, jobJoin.get("department"), searchPattern)
                ));
            }

            // Job-related filters
            SpecificationUtils.addPredicateIfNotEmpty(predicates, jobJoin.get("id"), filter.getJobId());
            SpecificationUtils.addPredicateIfNotEmpty(predicates, jobJoin.get("locationType"), filter.getLocationType());
            SpecificationUtils.addPredicateIfNotEmpty(predicates, jobJoin.get("employmentType"), filter.getEmploymentType());
            SpecificationUtils.addPredicateIfNotEmpty(predicates, jobJoin.get("status"), filter.getJobStatus());
