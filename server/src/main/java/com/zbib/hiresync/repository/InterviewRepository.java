...
//
//    @Query("SELECT i FROM Interview i WHERE i.status = :status AND i.scheduledStartTime > :now AND i.reminderSent = false")
//    List<Interview> findUpcomingInterviewsForReminders(InterviewStatus status, LocalDateTime now);
}